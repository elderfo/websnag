...
interface RequestRowProps {
  ...
  onCheckChange: (requestId: string, checked: boolean) => void
}
...
export function RequestRow({
  ...
  return (
    <div
      className={`flex items-center w-full border-b border-border transition-all duration-200 hover:bg-surface-hover ${
        isSelected
      ...
      {showCheckbox && (
        <label className="flex items-center pl-3 cursor-pointer">
          <input
            ...
            className="h-3.5 w-3.5 rounded border-border bg-background text-accent focus:ring-accent"
            aria-label={`Select request ${request.method} ${request.content_type ?? ''}`}
          />
        </label>
      )}
      <button
